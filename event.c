...
#ifdef DEBUG_EVENT
#ifndef EVENT_USE_POLL
static void print_select_sets(char const *tag,
			      int maxfdp1, fd_set *rd, fd_set *wr);
#endif
#endif
...
#ifndef EVENT_USE_POLL
/**********************************************************************
* %FUNCTION: Event_HandleEventUsingSelect
* %ARGUMENTS:
*  es -- EventSelector
* %RETURNS:
...
***********************************************************************/
int
Event_HandleEventUsingSelect(EventSelector *es)
{
	...
	for(;;) {
#ifdef DEBUG_EVENT
	    print_select_sets("before", maxfd+1, rd, wr);
#endif
	    r = select(maxfd+1, rd, wr, NULL, tm);
#ifdef DEBUG_EVENT
	    print_select_sets("after ", maxfd+1, rd, wr);
#endif
	    if (r < 0) {
...
#ifndef EVENT_USE_POLL
static void
print_select_sets(char const *tag, int maxfdp1, fd_set *rd, fd_set *wr)
{
    int fd;
    int done;

    if (!Event_DebugFP) return;
    fprintf(Event_DebugFP, "%s select: rd = [", tag);
    done = 0;
    if (rd) {
	for (fd=0; fd<maxfdp1; fd++) {
	    if (FD_ISSET(fd, rd)) {
		if (done) fprintf(Event_DebugFP, ", ");
		done = 1;
		fprintf(Event_DebugFP, "%d", fd);
	    }
	}
    }
    fprintf(Event_DebugFP, "]\n%s select: wr = [", tag);
    done = 0;
    if (wr) {
	for (fd=0; fd<maxfdp1; fd++) {
	    if (FD_ISSET(fd, wr)) {
